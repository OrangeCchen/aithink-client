...
> **版本**: v0.4  
> **日期**: 2026-08-06  
> **状态**: P1 Mock 完成；P2 改为「已打开 App + 魔法鼠标」，CDP spawn 路径已废弃删除  
...
- **短期**：执行面 = 用户**已打开且已登录**的 Agent 产品（先做 QoderWork）；调度台**不** spawn 独立调试实例，用辅助功能 + 魔法箭头操控日常窗口。
- **长期**：执行面扩到**任意已授权 App**（Computer Use 同款）；编排层不变。
...
**重要发现（P0 历史记录）**：当时探测表明正常启动默认不开调试端口。  
**P2 纠偏（2026-08-06）**：不再依赖调试端口 / spawn；改为操控用户已打开的日常实例（辅助功能 + 魔法箭头）。上表 CDP 结论仅作技术档案保留。
...
| **P2 · 第一个闭环** | 操控**已打开**的 QoderWork：魔法箭头 + AX 填 prompt → 完成检测 → 抽结果 | 单个 Agent（a11y） | 不启动第二实例；日常登录态可用 |
| **P3 · 接第 2～3 家** | 检验 `AppAdapter` 抽象 | 多家特化 Agent | 抽象稳定或据此重构 |
| **P4 · 任意已打开 App** | 通用 AX / 视觉兜底（对标 `@Computer`） | 扩大执行面 | 无 CDP 要求 |
| **贯穿** | 授权向导、魔法箭头、暂停/接管；（PiP 可选） | 可观测性 | — |

> **已废弃**：`spawn --remote-debugging-port` + puppeteer CDP 主路径（独立 user-data-dir 无登录态，且 0.9+ 端口行为不稳）。相关代码已删除。
...
1. **近期（P2）**：用户先打开并登录目标 App；调度台通过 `native/mac-computer-use` 辅助功能操控，**不**自动启动。
2. **权限**：macOS **辅助功能**必开（给 AIThink / helper）；截图兜底时再要屏幕录制。
3. **`ensureReady()`**：未打开 / 未授权 → 任务失败并提示，绝不 spawn 调试实例。
...
| 2026-08-05 | v0.3 | 同步调研结论：工具=派、短期特化 Agent → 长期任意 App；PiP/魔法箭头 |
| 2026-08-06 | v0.4 | **纠偏**：废弃 CDP spawn；P2 改为已打开 App + 魔法鼠标（AX helper） |
