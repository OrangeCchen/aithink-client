 ...
 * 保存授权记录（用于显示列表）
 */
async function savePermissionRecord(origin: string, hostname: string): Promise<void> {
 ...
 * 获取所有授权记录
 */
export async function getPermissionRecords(): Promise<PermissionInfo[]> {
 ...
 * 移除权限
 */
export async function removePermission(origin: string): Promise<boolean> {
  try {
 ...
 * 检查并请求权限（如果需要）
 */
export async function ensurePermission(url: string): Promise<{ ok: boolean; message?: string }> {
