...
export function normalizeDesignLink(raw: string): string {
  ...
  try {
    ...
    [...u.searchParams.entries()]
      ...
      .map(([k, v]) => [k, stripInvisibles(v)] as [string, string])
...
export function dedupeDesignLinks(links: Iterable<string>): string[] {
