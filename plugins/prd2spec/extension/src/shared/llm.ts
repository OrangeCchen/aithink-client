import type { DesignCapture, PrdContent, Provider, UserSettings } from './types';
import { buildUserPrompt, SPEC_SYSTEM_PROMPT, SPEC_TEST_SYSTEM_PROMPT } from './prompts';

export interface ChatMessage {
  role: 'user' | 'assistant';
  content: string;
  images?: string[]; // dataURLs，仅 user 消息发送时使用；assistant 消息忽略
}
...
export function getApiKey(settings: UserSettings, provider: Provider): string {
  return provider === 'qwen' ? settings.qwenApiKey : settings.anthropicApiKey;
}

function dataUrlToBase64(dataUrl: string): { mediaType: string; base64: string } | null {
  const match = /^data:([^;]+);base64,(.+)$/.exec(dataUrl);
  if (!match) return null;
  return { mediaType: match[1], base64: match[2] };
}

function buildAnthropicContent(prd: PrdContent, designs: DesignCapture[]): AnthropicContentBlock[] {
...
function buildOpenAIContent(prd: PrdContent, designs: DesignCapture[]): OpenAIContentPart[] {
  const totalShots = designs.reduce((sum, d) => sum + d.screenshots.length, 0);
  const hasAnnotations = designs.some((d) => d.annotations);
...
async function streamAnthropic(
  ...
  try {
    ...
    callbacks.onError(`网络请求失败：${(err as Error).message}`);
    return;
  }
  ...
  try {
    ...
    callbacks.onError(`流读取失败：${(err as Error).message}`);
  }
}
...
export async function streamGenerateSpec(
  apiKey: string,
  model: string,
  prd: PrdContent,
  designs: DesignCapture[],
...
export async function testApiKey(
  ...
  if (provider === 'anthropic') {
    try {
      ...
      if (res.ok) return { ok: true, message: `连通成功（${model}）` };
...
function buildAnthropicChatMessages(messages: ChatMessage[]) {
...
function buildOpenAIChatMessages(messages: ChatMessage[]) {
  return messages.map((m) => {
    if (!m.images?.length) return { role: m.role, content: m.content };
    const parts: OpenAIContentPart[] = [];
