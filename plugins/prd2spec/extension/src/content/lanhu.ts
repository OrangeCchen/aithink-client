...
async function waitForCanvasReady(timeoutMs = 8000): Promise<void> {
...
function extractAnnotationText(): string {
...
function getAllCanvases(): HTMLCanvasElement[] {
...
function captureCanvases(): string[] {
  ...
  for (const { c } of sorted) {
    ...
    try {
      ...
      if (dataUrl && dataUrl !== 'data:,') shots.push(dataUrl);
    } catch {
      // canvas 跨域或被污染，跳过
    }
  }
...
async function captureDesign(url: string): Promise<DesignCapture> {
