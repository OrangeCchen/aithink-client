...
interface MasterGoMcpParams {
  fileId: string;
  layerId?: string;
}

function parseMasterGoUrl(url: string): MasterGoMcpParams | null {
  try {
    const u = new URL(url);
    if (!u.hostname.includes('mastergo')) return null;
...
async function fetchMasterGoMcpDsl(params: MasterGoMcpParams, token: string, originalUrl: string): Promise<string> {
...
function findContentScriptFiles(hint: string): string[] | null {
  const manifest = chrome.runtime.getManifest();
  const cs = manifest.content_scripts?.find((s) =>
    s.js?.some((path) => path.includes(hint)),
  );
  return cs?.js ?? null;
}
...
async function sendMessageWithInjection<TReq, TRes>(
  tabId: number,
  message: TReq,
  scriptHint: string,
): Promise<TRes> {
  try {
    return await chrome.tabs.sendMessage<TReq, TRes>(tabId, message);
  } catch (err) {
  ...
  try {
    ...
    const ie = (injectErr as Error).message ?? '';
    throw new Error(
      [
        '无法在当前页面注入提取脚本。请确认：',
  ...
  try {
    ...
    throw new Error(`注入成功但页面仍无响应：${(retryErr as Error).message}`);
  }
}

async function requestPrdFromActiveTab(): Promise<PrdContent> {
  const tab = await getActiveTab();
  if (!tab?.id) throw new Error('找不到当前标签页');
  const response = await sendMessageWithInjection<AnyMessage, ExtractPrdResultMessage>(
    tab.id,
    { type: 'EXTRACT_PRD' },
    'feishu',
  );
...
async function captureDesignByOpeningTab(designUrl: string): Promise<DesignCapture> {
  ...
  if (!created.id) throw new Error('无法打开设计稿标签页');
  ...
  let result: DesignCapture;
  try {
    const response = await chrome.tabs.sendMessage<AnyMessage, CaptureDesignResultMessage>(
      tabId,
      { type: 'CAPTURE_DESIGN', url: designUrl },
    );
    ...
    try {
      ...
      throw new Error(
        `无法抓取该设计稿页面：${(err as Error).message}；并且后台截图也失败：${(capErr as Error).message}。可能原因：该域名未在扩展 host_permissions 内，或页面需要登录。请告诉作者把域名加入 manifest。`,
      );
    } finally {
...
async function handleGenerate(msg: GenerateSpecMessage): Promise<void> {
  const settings = await loadSettings();
...
async function handleWriteBack(msg: WriteBackMessage): Promise<WriteBackResultMessage> {
  const tab = await getActiveTab();
  if (!tab?.id) {
    return { type: 'WRITE_BACK_RESULT', payload: { ok: false, error: '找不到当前标签页' } };
  }
  try {
    const response = await chrome.tabs.sendMessage<WriteBackMessage, WriteBackResultMessage>(
      tab.id,
      msg,
    );
...
async function handleLocate(msg: LocateInDocMessage): Promise<LocateInDocResultMessage> {
  const tab = await getActiveTab();
  if (!tab?.id) {
    return { type: 'LOCATE_IN_DOC_RESULT', payload: { ok: false, error: '找不到当前标签页' } };
  }
  try {
    const response = await sendMessageWithInjection<LocateInDocMessage, LocateInDocResultMessage>(
      tab.id,
      msg,
      'feishu',
    );
...
chrome.runtime.onMessage.addListener((rawMsg, _sender, sendResponse) => {
  ...
  const recMsg = rawMsg as { type?: string; recordingId?: string };
    ...
    captureDesignByOpeningTab(msg.url)
      .then((payload) =>
        sendResponse({
          ...
          requestId: msg.requestId,
          payload,
        } satisfies CaptureDesignResultMessage),
      )
      .catch((err: Error) =>
        sendResponse({
          type: 'CAPTURE_DESIGN_RESULT',
...
async function setRecordingState(state: RecordingState | null): Promise<void> {
  if (state === null) {
...
async function updateBadge(state: RecordingState | null): Promise<void> {
  if (state) {
...
// 上报页面访问到桌面端
async function trackPage(url: string, title: string): Promise<void> {
